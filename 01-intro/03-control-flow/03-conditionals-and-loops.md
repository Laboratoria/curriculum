...
Como muestra el ejemplo a continuación, una sentencia `if` tiene 2 partes principales: la condición y el cuerpo. La condición debe ser un valor `Boolean` que va entre paréntesis. El cuerpo consiste en una o más sentencias JavaScript que se ejecutarán si, y solo si, la condición es verdadera (`Boolean` es igual a `true`).
...
El programa primero revisará si `num` es menor que 10. Si lo es, escoge ese camino, muestra "Diste un número pequeño" en un alert y termina. Si no lo es, toma el camino del `else`, que en sí mismo contiene un segundo `if`. Si la segunda condición (< 100) se cumple, significa que el número está entre 10 y 100, y se muestra "Diste un número mediano" en un alert. Si no lo es, el segundo y último `else` es escogido, mostrabdo "Diste un número grande".
...
Otra forma de modificar el flujo de un programa es estructuras repetitivas, también llamadas `bucles` ("loops" en inglés). Tal como hemos visto, los condicionales permiten ejecutar un conjunto de sentencias una vez si una condición _**es verdadera**_. Los bucles, permiten ejecutar un código varias veces, dependiendo de si una condición _**sigue siendo verdadera**_.

Piensa en un programa que imprima todos los números pares del 0 al 12. Una manera de escribirlo sería como sigue:
...
Regresemos al problema de imprimir todos los números pares del 0 al 12, y creemos un programa que utilice un bucle `while`:
...
- ¿Cómo hacemos para imprimir todos los números **pares** e **impares** del 0 al 12?: En la segunda sentencia del while, incrementamos `number` en 1 (en lugar de 2) cada ciclo.
...
La palabra clave `continue` es similar a break en que influencia el progreso del bucle. Cuando se encuentra `continue` en el cuerpo de un bucle, el control sale del cuerpo del bucle inmediatamente y continúa en la próxima iteración del bucle.
