...
describe("findShortestWordAmongMixedElements()", function () {
  it("deberia regresar '', para  ([3, 4, 5])", function () {
	var output = findShortestWordAmongMixedElements([3, 4, 5]);
	Assert.deepEqual(output, '');
  });

  it("deberia regresar '', para ([])", function () {
	var output = findShortestWordAmongMixedElements([]);
	Assert.deepEqual(output, '');
  });
  

  it("deberia regresar 'yes', para ([1, 2, 3, 'yes', 'one')", function () {
  ...
  it("deberia regresar 'two', para [4, 'two', 2, 'three'])", function () {
