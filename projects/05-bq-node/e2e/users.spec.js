...
describe('POST /users', () => {
  ...
  it('should respond with 400 when email is missing', () => (
    fetch('/users', { method: 'POST', body: { email: '', password: 'xxxx' } })
      .then(resp => expect(resp.status).toBe(400))
  ));

  it('should respond with 400 when password is missing', () => (
    fetch('/users', { method: 'POST', body: { email: 'foo@bar.baz' } })
      .then(resp => expect(resp.status).toBe(400))
  ));
  ...
  it('should create new user', () => {
    // this test is kind of redundant as we have already created a user in the
    // `globalSetup` script...
    // createTestUser().then(user => user.remove())
  });
...
describe('DELETE /users/:uid', () => {
  ...
  it('should delete own user', () => {
    ...
    return fetch('/users', { method: 'POST', body: credentials })
      .then(resp => expect(resp.status).toBe(200))
