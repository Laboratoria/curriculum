...
describe('GET /products/:productid', () => {
  it('should fail with 404 when not found', () =>
    fetchAsTestUser('/products/notarealproduct').then((resp) =>
...
describe('PUT /products/:productid', () => {
  it('should fail with 401 when no auth', () =>
    fetch('/products/xxx', { method: 'PUT' }).then((resp) =>
      expect(resp.status).toBe(401),
    ));
...
describe('DELETE /products/:productid', () => {
  it('should fail with 401 when no auth', () =>
    fetch('/products/xxx', { method: 'DELETE' }).then((resp) =>
      expect(resp.status).toBe(401),
    ));
  ...
  it('should fail with 403 when not admin', () =>
    fetchAsAdmin('/products', {
      ...
      .then((json) =>
        fetchAsTestUser(`/products/${json._id}`, { method: 'DELETE' }),
      )
      .then((resp) => expect(resp.status).toBe(403)));

  it('should fail with 404 when admin and not found', () =>
    fetchAsAdmin('/products/12345678901234567890', { method: 'DELETE' }).then(
  ...
  it('should delete other product as admin', () =>
    fetchAsAdmin('/products', {
      ...
      .then(({ _id }) =>
        fetchAsAdmin(`/products/${_id}`, { method: 'DELETE' }).then((resp) => ({
          resp,
          _id,
        })),
      )
