...
self.importScripts('https://cdnjs.cloudflare.com/ajax/libs/mocha/10.2.0/mocha.min.js', 'https://cdnjs.cloudflare.com/ajax/libs/chai/4.3.7/chai.min.js', 'https://cdnjs.cloudflare.com/ajax/libs/sinon.js/14.0.2/sinon.min.js');
mocha.setup({
  ui: 'bdd',
...
`);
const wrapTests = str => new Function('requires', `var require = (name) => {
...
const loadTests = ({
  ...
  wrapTests(tests)({
    chai: () => chai,
    sinon: () => sinon,
    [`../solution/${fname.replace(/\.js$/, '')}`]: wrapSubmission(code)
  });
};
