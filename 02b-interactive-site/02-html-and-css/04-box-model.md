...
Su valor se puede definir en: [Unidades de medida](http://librosweb.es/referencia/css/unidades_de_medida.html), porcentaje o inherit(especifica que el padding debe ser heredado del elemento padre).
...
- Si solo se indica un valor, todos los rellenos tienen ese valor.
```CSS
div {
  padding: 2em;
}     
...
```
- Si se indican dos valores, el primero se asigna al relleno superior e inferior y el segundo se asigna al derecho e izquierdo.
```CSS
div {
  padding: 1em 2em;
}     
...
```
- Si se indican tres valores, el primero se asigna al relleno superior, el segundo valor se asigna al derecho e izquierdo; y el tercero, se asigna al relleno inferior.
```CSS
div {
  padding: 1em 2em 3em;
}     
...
```
- Si se indican los cuatro valores, se asignarán los rellenos en orden horario, de la siguiente forma: superior, derecho, inferior e izquierdo.
```CSS
div {
...
### Border
Es la línea que encierra el contenido y el padding. Es aplicable a todos los elementos.
...
La anchura de los bordes se indica mediante una [unidad de medida](http://librosweb.es/referencia/css/unidades_de_medida.html) o mediante las palabras clave `thin` (borde delgado), `medium` (borde normal) y `thick` (borde ancho).
...
Los valores que puede tomar son: color, transparent o inherit.
...
Su valor se puede definir en: [Unidades de medida](http://librosweb.es/referencia/css/unidades_de_medida.html) + color de borde + estilo de borde, o inherit(especifica que el borde debe ser heredado del elemento padre).
...
```
>De esta forma, con la propiedad `border` hemos asignado el ancho, color y estilo de borde para todos los cuatro lados de la caja del elemento `div`.

También, tenemos la posibilidad de asignar los valores(ancho, color y estilo) para un lado en especial de la siguiente manera:
```CSS
div {
...
Su valor se puede definir en: [Unidades de medida](http://librosweb.es/referencia/css/unidades_de_medida.html), porcentaje o inherit(especifica que el margin debe ser heredado del elemento padre).
...
Es la imagen que se muestra por detrás del contenido y el espacio de relleno.

##### Background color (___color de fondo___)
Es el color que se muestra por detrás del contenido y el espacio de relleno.
...
Su valor se puede definir en: [Unidades de medida](http://librosweb.es/referencia/css/unidades_de_medida.html), porcentaje, auto o inherit. Siendo como valor inicial _auto_.
...
Su valor se puede definir en: [Unidades de medida](http://librosweb.es/referencia/css/unidades_de_medida.html), porcentaje, auto o inherit. Siendo como valor inicial _auto_.
Al igual que sucede con width, la propiedad height no admite valores negativos. Si se indica un porcentaje, se toma como referencia la altura del elemento padre.
